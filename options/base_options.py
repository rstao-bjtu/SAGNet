...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--dataset1_classes', default='', help='image classes to train on')
        parser.add_argument('--dataset2_classes', default='', help='image classes to train on')
        ...
        parser.add_argument('--pth_dataroot', default='', help='path to pth to test')
    ...
    def parse(self, print_options=True):
        ...
        opt.name = '__'.join([opt.name, time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()), 'lnum_'+str(opt.lnum),thePth])
        # process opt.suffix
        if opt.suffix:
        ...
        opt.dataset1_classes = opt.dataset1_classes.split(',')
        opt.dataset2_classes = opt.dataset2_classes.split(',')
